package routes

import (
	"time"

	"github.com/gofiber/fiber/v2"
	"github.com/google/uuid"

	"UAS/app/models"
	"UAS/app/repository"
)

// AchievementListHandler handles listing achievements
// GET /api/v1/achievements
// - Admin: lihat semua
// - Mahasiswa: lihat milik diri sendiri
// - Dosen Wali: lihat anak bimbingannya
func AchievementListHandler(c *fiber.Ctx) error {
	userID := c.Locals("user_id")
	role := c.Locals("role")
	achievementRepo := repository.NewAchievementRepository()

	var achievements []models.Achievement
	var err error

	switch role {
	case "Admin":
		// Admin lihat semua
		achievements, err = achievementRepo.FindAll()
	case "Mahasiswa":
		// Mahasiswa lihat miliknya sendiri
		achievements, err = achievementRepo.FindByStudentID(userID.(string))
	case "Dosen Wali":
		// TODO: Dosen Wali lihat anak bimbingannya
		// For now, return empty for Dosen Wali
		achievements = []models.Achievement{}
	default:
		achievements, err = achievementRepo.FindAll()
	}

	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"status": "error",
			"error":  "failed to fetch achievements",
		})
	}

	return c.Status(fiber.StatusOK).JSON(fiber.Map{
		"status": "success",
		"data":   achievements,
	})
}

// AchievementDetailHandler handles getting achievement detail
// GET /api/v1/achievements/:id
// - Semua role bisa lihat, dengan ownership check untuk Mahasiswa
func AchievementDetailHandler(c *fiber.Ctx) error {
	id := c.Params("id")
	userID := c.Locals("user_id")
	role := c.Locals("role")
	achievementRepo := repository.NewAchievementRepository()

	achievement, err := achievementRepo.FindByID(id)
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"status": "error",
			"error":  "achievement not found",
		})
	}

	// Ownership check
	if role == "Mahasiswa" && achievement.StudentID != userID.(string) {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
			"status": "error",
			"error":  "you can only view your own achievements",
		})
	}

	// TODO: Dosen Wali check - verify mahasiswa is under their guidance
	if role == "Dosen Wali" {
		// For now allow all
	}

	return c.Status(fiber.StatusOK).JSON(fiber.Map{
		"status": "success",
		"data":   achievement,
	})
}

// AchievementCreateHandler handles creating/submitting achievement (FR-003)
// POST /api/v1/achievements
// Mahasiswa & Admin: bisa submit prestasi
// Status awal: draft
func AchievementCreateHandler(c *fiber.Ctx) error {
	userID := c.Locals("user_id")
	role := c.Locals("role")
	var req models.AchievementCreateRequest

	if err := c.BodyParser(&req); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"status": "error",
			"error":  "invalid request body",
		})
	}

	// Validate required fields
	if req.Title == "" || req.Category == "" || req.Date == "" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"status": "error",
			"error":  "title, category, and date are required",
		})
	}

	// Only Mahasiswa and Admin can create
	if role != "Mahasiswa" && role != "Admin" {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
			"status": "error",
			"error":  "only mahasiswa and admin can submit achievements",
		})
	}

	// Parse date
	achievementDate, err := time.Parse("2006-01-02", req.Date)
	if err != nil {
		achievementDate = time.Now()
	}

	achievement := &models.Achievement{
		ID:          uuid.New().String(),
		StudentID:   userID.(string),
		Title:       req.Title,
		Description: req.Description,
		Category:    req.Category,
		Date:        achievementDate,
		ProofURL:    req.ProofURL,
		Status:      "draft",
		CreatedAt:   time.Now(),
		UpdatedAt:   time.Now(),
	}

	achievementRepo := repository.NewAchievementRepository()
	if err := achievementRepo.Create(achievement); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"status": "error",
			"error":  "failed to create achievement",
		})
	}

	return c.Status(fiber.StatusCreated).JSON(fiber.Map{
		"status":  "success",
		"message": "Prestasi berhasil disubmit",
		"data":    achievement,
	})
}

// AchievementUpdateHandler handles updating achievement
// PUT /api/v1/achievements/:id
// Mahasiswa bisa update miliknya sendiri (hanya status draft)
// Admin bisa update semua
func AchievementUpdateHandler(c *fiber.Ctx) error {
	id := c.Params("id")
	userID := c.Locals("user_id")
	role := c.Locals("role")
	var req models.AchievementUpdateRequest

	if err := c.BodyParser(&req); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"status": "error",
			"error":  "invalid request body",
		})
	}

	achievementRepo := repository.NewAchievementRepository()
	achievement, err := achievementRepo.FindByID(id)
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"status": "error",
			"error":  "achievement not found",
		})
	}

	// Ownership check
	if role == "Mahasiswa" {
		if achievement.StudentID != userID.(string) {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
				"status": "error",
				"error":  "you can only update your own achievements",
			})
		}
		// Mahasiswa hanya bisa update draft
		if achievement.Status != "draft" {
			return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
				"status": "error",
				"error":  "you can only update achievements in draft status",
			})
		}
	}

	// Update fields
	if req.Title != "" {
		achievement.Title = req.Title
	}
	if req.Description != "" {
		achievement.Description = req.Description
	}
	if req.Category != "" {
		achievement.Category = req.Category
	}
	if req.Date != "" {
		if date, err := time.Parse("2006-01-02", req.Date); err == nil {
			achievement.Date = date
		}
	}
	if req.ProofURL != "" {
		achievement.ProofURL = req.ProofURL
	}

	achievement.UpdatedAt = time.Now()

	if err := achievementRepo.Update(id, achievement); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"status": "error",
			"error":  "failed to update achievement",
		})
	}

	return c.Status(fiber.StatusOK).JSON(fiber.Map{
		"status":  "success",
		"message": "Achievement updated successfully",
		"data":    achievement,
	})
}

// AchievementDeleteHandler handles deleting achievement
// DELETE /api/v1/achievements/:id
// Mahasiswa bisa delete miliknya sendiri
// Admin bisa delete semua
func AchievementDeleteHandler(c *fiber.Ctx) error {
	id := c.Params("id")
	userID := c.Locals("user_id")
	role := c.Locals("role")
	achievementRepo := repository.NewAchievementRepository()

	achievement, err := achievementRepo.FindByID(id)
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"status": "error",
			"error":  "achievement not found",
		})
	}

	// Ownership check
	if role == "Mahasiswa" && achievement.StudentID != userID.(string) {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
			"status": "error",
			"error":  "you can only delete your own achievements",
		})
	}

	if err := achievementRepo.Delete(id); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"status": "error",
			"error":  "failed to delete achievement",
		})
	}

	return c.Status(fiber.StatusOK).JSON(fiber.Map{
		"status":  "success",
		"message": "Achievement deleted successfully",
	})
}

// AchievementSubmitHandler handles submitting achievement for verification
// POST /api/v1/achievements/:id/submit
// Status: draft → submitted
func AchievementSubmitHandler(c *fiber.Ctx) error {
	id := c.Params("id")
	userID := c.Locals("user_id")
	role := c.Locals("role")
	achievementRepo := repository.NewAchievementRepository()

	achievement, err := achievementRepo.FindByID(id)
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"status": "error",
			"error":  "achievement not found",
		})
	}

	// Ownership check
	if role == "Mahasiswa" && achievement.StudentID != userID.(string) {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
			"status": "error",
			"error":  "you can only submit your own achievements",
		})
	}

	// Can only submit draft
	if achievement.Status != "draft" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"status": "error",
			"error":  "only draft achievements can be submitted",
		})
	}

	if err := achievementRepo.UpdateStatus(id, "submitted"); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"status": "error",
			"error":  "failed to submit achievement",
		})
	}

	return c.Status(fiber.StatusOK).JSON(fiber.Map{
		"status":  "success",
		"message": "Achievement submitted successfully",
		"data": fiber.Map{
			"id":     id,
			"status": "submitted",
		},
	})
}

// AchievementVerifyHandler handles verifying achievement
// POST /api/v1/achievements/:id/verify
// Dosen Wali & Admin: bisa verify
func AchievementVerifyHandler(c *fiber.Ctx) error {
	id := c.Params("id")
	role := c.Locals("role")
	var req models.AchievementVerifyRequest

	if err := c.BodyParser(&req); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"status": "error",
			"error":  "invalid request body",
		})
	}

	// Only Dosen Wali and Admin can verify
	if role != "Dosen Wali" && role != "Admin" {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
			"status": "error",
			"error":  "only dosen wali and admin can verify achievements",
		})
	}

	achievementRepo := repository.NewAchievementRepository()
	achievement, err := achievementRepo.FindByID(id)
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"status": "error",
			"error":  "achievement not found",
		})
	}

	// Can only verify submitted
	if achievement.Status != "submitted" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"status": "error",
			"error":  "only submitted achievements can be verified",
		})
	}

	achievement.Status = "verified"
	achievement.Points = req.Points
	achievement.UpdatedAt = time.Now()

	if err := achievementRepo.Update(id, achievement); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"status": "error",
			"error":  "failed to verify achievement",
		})
	}

	return c.Status(fiber.StatusOK).JSON(fiber.Map{
		"status":  "success",
		"message": "Achievement verified successfully",
		"data": fiber.Map{
			"id":     id,
			"status": "verified",
			"points": req.Points,
		},
	})
}

// AchievementRejectHandler handles rejecting achievement
// POST /api/v1/achievements/:id/reject
// Dosen Wali & Admin: bisa reject
func AchievementRejectHandler(c *fiber.Ctx) error {
	id := c.Params("id")
	role := c.Locals("role")
	var req models.AchievementRejectRequest

	if err := c.BodyParser(&req); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"status": "error",
			"error":  "invalid request body",
		})
	}

	// Only Dosen Wali and Admin can reject
	if role != "Dosen Wali" && role != "Admin" {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
			"status": "error",
			"error":  "only dosen wali and admin can reject achievements",
		})
	}

	achievementRepo := repository.NewAchievementRepository()
	achievement, err := achievementRepo.FindByID(id)
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"status": "error",
			"error":  "achievement not found",
		})
	}

	// Can only reject submitted
	if achievement.Status != "submitted" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"status": "error",
			"error":  "only submitted achievements can be rejected",
		})
	}

	achievement.Status = "rejected"
	achievement.RejectionNote = req.Reason
	achievement.UpdatedAt = time.Now()

	if err := achievementRepo.Update(id, achievement); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"status": "error",
			"error":  "failed to reject achievement",
		})
	}

	return c.Status(fiber.StatusOK).JSON(fiber.Map{
		"status":  "success",
		"message": "Achievement rejected",
		"data": fiber.Map{
			"id":     id,
			"status": "rejected",
			"reason": req.Reason,
		},
	})
}

// AchievementHistoryHandler handles getting achievement status history
// GET /api/v1/achievements/:id/history
// Semua role bisa lihat
func AchievementHistoryHandler(c *fiber.Ctx) error {
	id := c.Params("id")

	// TODO: Implement history tracking
	// For now, return empty history
	return c.Status(fiber.StatusOK).JSON(fiber.Map{
		"status": "success",
		"data": fiber.Map{
			"id":      id,
			"history": []fiber.Map{},
		},
	})
}

// AchievementUploadAttachmentHandler handles uploading files to achievement
// POST /api/v1/achievements/:id/attachments
// Mahasiswa & Admin: bisa upload
func AchievementUploadAttachmentHandler(c *fiber.Ctx) error {
	id := c.Params("id")
	userID := c.Locals("user_id")
	role := c.Locals("role")

	// Only Mahasiswa and Admin can upload
	if role != "Mahasiswa" && role != "Admin" {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
			"status": "error",
			"error":  "only mahasiswa and admin can upload files",
		})
	}

	// Handle file upload
	file, err := c.FormFile("file")
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"status": "error",
			"error":  "file is required",
		})
	}

	achievementRepo := repository.NewAchievementRepository()
	achievement, err := achievementRepo.FindByID(id)
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"status": "error",
			"error":  "achievement not found",
		})
	}

	// Ownership check for Mahasiswa
	if role == "Mahasiswa" && achievement.StudentID != userID.(string) {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{
			"status": "error",
			"error":  "you can only upload files to your own achievements",
		})
	}

	// TODO: Implement file upload to storage
	// For now, just update proof_url with filename
	achievement.ProofURL = file.Filename
	achievement.UpdatedAt = time.Now()

	if err := achievementRepo.Update(id, achievement); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"status": "error",
			"error":  "failed to upload file",
		})
	}

	return c.Status(fiber.StatusCreated).JSON(fiber.Map{
		"status":  "success",
		"message": "File uploaded successfully",
		"data": fiber.Map{
			"id":       id,
			"filename": file.Filename,
			"size":     file.Size,
		},
	})
}
